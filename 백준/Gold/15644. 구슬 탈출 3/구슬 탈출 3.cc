...
int main() {
  queue<tuple<pair<int, int>, pair<int, int>, int, string>> Q;
    ...
    for(int i=0;i<4;i++){
      if(who_first(red, blue, i)){
        ...
        if(is_hall(tmp_red) && !is_hall(tmp_blue)){
          cnt = count+1; answer = hist+ch[i]; break;
        }
        if(!visit[tmp_red.first][tmp_red.second][tmp_blue.first][tmp_blue.second]){
          ...
          Q.push({tmp_red, tmp_blue, count+1, hist+ch[i]});
        }
      }
      else{
        tmp_blue = move(blue, i);
